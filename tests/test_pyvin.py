...
from .vin_samples import INVALID, DECODED_TOYOTA_COROLLA, TOYOTA_COROLLA, HYUNDAI_ELANTRA, BATCH
...
class TestPyVin(unittest.TestCase):
    ...
    def test_invalid_in_multi(self):
        ...
        vins = VIN(*samples)
        self.assertEqual(len([x for x in samples if x is not INVALID]), len(vins))
